...
The implemented CLI currently supports preferential allotments (`PREF`) and QIPs (`QIP`) by fetching filing metadata from NSE, downloading linked XBRL documents, fetching four-level industry mapping from `eggmasonvalue/stock-industry-map-in`, fetching Current Market Price (CMP) for stock symbols, and writing normalized JSON output. Robust retry mechanisms (via `tenacity`) ensure consistency during API flakes.

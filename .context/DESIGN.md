...
- For insider trading,
  ...
  - "Invocation of pledge" was historically omitted from the `sell` preset due to severely inconsistent filing practices where filers mix and match `transactionType` (e.g. `Sell`, `Pledge`, `Pledge Revoke`, `Pledge Invoke`) and `acqMode` (e.g. `Invocation of pledge`, `Pledge Creation`, `Revokation of Pledge`). We now handle this via a dedicated `forced-sales` preset.
  - The `forced-sales` preset looks for `"pledge invoke"` in `transactionDirection` OR `"invocation"` in `transactionMode`. To avoid double-counting overlaps (e.g. when a filer incorrectly submits an identical transaction under both "Sell" and "Pledge Invoke"), it inherently applies deduplication using a composite key of `(symbol, personName, transactionQuantity, transactionStartDate)`.
